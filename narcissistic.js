...
function narcissistic(value) {
  ...
  let realDigits = splitArray.map((x) => Number(x));
  let powNums = realDigits.map((x) => Math.pow(x, realDigits.length));
  let testPow = powNums.reduce((accum, current) => accum + current, 0);
  let bool = value === testPow ? true : false;
  return bool;
}
